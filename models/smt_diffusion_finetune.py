...
class SMT_Diffusion_Finetune(nn.Module):
    ...
    def freeze_transferlearning_step_stage(self):
        for name, param in self.named_parameters():
            if self.finetune_mode == 'stage0':
                if 'block1' not in name:
                    param.requires_grad = False
                else:
                    param.requires_grad = True

                if 'patch_embed2' in name:
                    param.requires_grad = True
                ...
                if 'block2' not in name:
                    param.requires_grad = False

                if 'patch_embed3' in name:
                    param.requires_grad = True

                if name.startswith('norm2'):
                    param.requires_grad = True
            elif self.finetune_mode == 'stage2':
                if 'block3' not in name:
                    param.requires_grad = False

                if 'patch_embed4' in name:
                    param.requires_grad = True

                if name.startswith('norm3'):
                    param.requires_grad = True
            elif self.finetune_mode == 'stage3':
                if 'block4' not in name:
                    param.requires_grad = False
            else:
                pass

            if 'patch_embed1' in name:
                param.requires_grad = True
...
def test(num_classes: int = 1000, **kwargs):
    ...
    model = SMT_Diffusion_Finetune(
        ...
        finetune_mode='sequence_stage3', is_efficient_finetune=True,
